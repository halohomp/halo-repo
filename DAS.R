# ==============================================================================
# SCRIPT MONITORING CURAH HUJAN HARIAN (SINGLE MAP - A4 SIZE)
# TANGGAL: 3 JUNI 2026
...
sungai      <- read_sf(paste0(path_shp, "RIV_SUMUT_CLIP.shp")) %>% st_set_crs(4326) # Layer Sungai Baru
...
# Proses data khusus untuk tanggal 3
...
peta_final <- ggplot() +
  ...
  # Layer 2: Jaringan Sungai (Warna biru pucat agar tidak menutupi stasiun)
  geom_sf(data = sungai, color = "#82C0E8", linewidth = 0.2, alpha = 0.7) +
  ...
  # Layer 5: Label Otomatis Aman untuk Curah Hujan Ekstrem (>100mm)
  geom_text_repel(
  ...
  scale_fill_manual(
    ...
    guide = guide_legend(
      ...
      nrow = 1 # Dibuat satu baris memanjang agar proporsional di A4 Landscape
    )
  ) +
...
# --- 8. PENYIMPANAN HASIL GRAFIK (UKURAN A4) ---
